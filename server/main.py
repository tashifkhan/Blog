...
if not MONGODB_URI:
    # The app can still start, but endpoints that need DB will 500 until set
    pass
...
@app.get("/health")
async def health():
    node = os.getenv("PYTHON_VERSION", "python")
    ...
    return {"ok": True, "runtime": node, "env": env, "mongo": mongo}
...
async def get_views(slug: str, request: Request):
    ...
    try:
        ...
        # don't block returning current count
        pass
...
async def like_post(slug: str):
    ...
    # PyMongo returns a document or None; extract likes
    if not res:
        # if None, fetch
...
async def add_comment(slug: str, payload: CommentIn):
    ...
    if not post:
        ...
        # reconstruct nested replies in app side
...
# Posts listing similar to Astro static endpoint
@app.get("/posts.json")
async def list_posts():
    # Path: server/main.py -> src/blogs
    # Compute project root two levels up from this file
    root = Path(__file__).resolve().parents[1]
    blogs_dir = root / "src" / "blogs"
    files = glob(str(blogs_dir / "*.md")) + glob(str(blogs_dir / "*.mdx"))
    posts = []
    for f in files:
        try:
