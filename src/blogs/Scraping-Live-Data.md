...
Scraping can violate a site's terms of service. The point of this post is understanding how detection layers work and how the web actually fits together — useful whether you're pulling data or defending it. Be polite: cache aggressively, respect rate limits, don't hammer origins.
</Note>
...
For Malabar this was a Magento GraphQL endpoint (<code>getMetalRate</code>). For Senco, a calculator API. For catalog pages, a Klevu search SaaS. For Next.js product pages, the <code>#__NEXT_DATA__</code> script tag — the entire price breakup, pre-serialized for hydration.
</Step>
...
Scraped 22K gets normalized with digit-length heuristics (per-gram vs per-10g — jewellers can never agree on units), then 24/18/14K are derived by purity ratios.
</Step>
</Steps>
...
DrissionPage, Playwright stealth, or FlareSolverr as a self-hosted solver microservice. Paid captcha solvers as the absolute last resort. Notably: <em>not used anywhere in Pitaara's live path</em> — we engineered around it.
</Step>
</Steps>
...
| `selectolax`              | Parse           | N/A          | —       | tiny           | DOM IDs, tables             |
| `chompjs`                 | Parse           | N/A          | —       | tiny           | JS object → dict            |
...
- [ ] Open DevTools Network on the real site — find the JSON/XHR the SPA already calls
- [ ] Remember: "no public API" ≠ no API — modern React apps must talk to something
...
- [ ] Cache results (Mongo / Redis) — never scrape on every UI hit
