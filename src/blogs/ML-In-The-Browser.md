...
Both run inference 100% client-side, with no server round-trip after the page loads. Every version number below comes from the Pyodide 314.0.4 lockfile.
...
If you take the Pyodide path, you pin your **training** environment to whatever your chosen Pyodide release ships. Not the other way around. Numbers below are for **Pyodide v314.0.4**, so re-check the [packages list](https://pyodide.org/en/stable/usage/packages-in-pyodide.html) before you train, because these drift.
...
The key browser code, which is basically the whole Pyodide path, condensed:
...
And it works. Cold start boots CPython, installs the scientific stack, mounts WordNet, and unpickles both models in **7–8.5 seconds** on my machine, run to run. Then each prediction is a genuinely correct call through the original pipeline. I verified it against the backend's own predictions on a golden set.

But look at the number. **Seven to eight and a half seconds.** That's a Web Worker's worth of "please wait," and it's the honest price of Path A.

<Warning title="Pickle the Pipeline, not the estimator">
If training did <code>StandardScaler → PCA → LogisticRegression</code> and you only pickled the last step, congratulations: you now get to reimplement every transform in JS, and you <em>will</em> ship training–serving skew. Same energy for <code>OneHotEncoder</code> (set <code>handle_unknown</code> at train time), <code>TfidfVectorizer</code> (huge vocab = WASM heap pain), and <code>FunctionTransformer</code> with a lambda (needs cloudpickle, still insecure, ONNX can't help you either, so rewrite the transform).
</Warning>
...
Same machine, same browser (Chrome, desktop), same model task. These are real numbers from the live site, measured via `performance.now()` in the page, in the same tab you can open right now.
...
Pickle records import paths. Custom classes must be importable inside Pyodide, so fetch the <code>.py</code> into the virtual FS, add it to <code>sys.path</code>, then import before <code>pickle.load</code>.
</Panel>
...
- [ ] Golden set: browser predictions match local within tolerance (mine: 100% ONNX-vs-sklearn agreement on 300 samples)
...
So which one won? **ONNX.** Not because the benchmark alone said so, but because the site made the decision for me.
...
- Preprocessing in JS means the *same* feature builder runs everywhere, across feed, search and categories, with one code path, no per-page Python round-trip.
